# setwd("Z:/jwardle/flight_spread/src/compare_peak_sizes")
# orderly::orderly_develop_start(use_draft = TRUE, parameters = list(init_inf = 100))

peak_sizes <- readRDS("peak_sizes.rds")
...
all_wider_repeat <- all_wider %>% 
  filter(pathogen == "Nat. history: SARS-CoV-2-like" & flight_scenario == "SARS-CoV-2")
...
coloured_plot <-
  ...
  labs(y = "Difference in mean size of epidemic peak, per 1000 people",
...
black_white_plot <-
  ...
  labs(y = "Difference in mean epidemic peak size\n(infections / 1000 people)",
...
ggsave("results_fig_peaksize_sarscov2.png", black_white_plot,
